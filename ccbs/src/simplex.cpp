    ...
    */
    ...
    void Simplex::set_problem(Matrix coefficients, std::vector<double> costs)
    {
        ...
        for(int i = 0; i < solution_dimension; i++)
        {
            ...
            add_variable(new Variable(std::to_string(i).c_str()));
        }
                ...
                if (token.length()) {
                    if (token == "[METADATA]")
                        ...
                        case PB_VARS:
                        {
                            ...
                            add_variable(new Variable(variable_name.c_str()));
    ...
    void Simplex::set_objective_function(ObjectiveFunction const& objective_function) {
        ...
        if (solution_dimension != objective_function.getCosts().dim().second)
    ...
    void Simplex::process_to_standard_form() {
        ...
        for (int i = 0; i < initial_solution_dimension; ++i) {                        // For each component of x
            ...
            if (!has_constraint) {
                ...
                objective_function.add_column(-objective_function.getCosts()(i));

                // Update variables status          
                string aux_name(variables.at(i)->getName());
                Variable* auxiliary = new AuxiliaryVariable((aux_name + "_minus").c_str(), variables.size());
                Variable* splitted = new SplittedVariable(variables.at(i)->getName().c_str(), (AuxiliaryVariable*)auxiliary);
        ...
        for (it = constraints.begin(); it != constraints.end(); ++it) {
            ...
            if (it->type == CT_MORE_EQUAL)  {
                ...
                variables.push_back(new SlackVariable(variable_name.str().c_str()));
        ...
        if (objective_function.getType() == OFT_MAXIMIZE) {
            objective_function.updateType(OFT_MINIMIZE);
            Matrix zero(1, solution_dimension, 0);
            changed_sign = true;
            // objective_function.costs = zero - objective_function.costs;
            objective_function.updateCosts(zero - objective_function.getCosts());
        }
    ...
    void Simplex::process_to_artificial_problem() {
        ...
        objective_function.makeCosts_empty();
    ...
    void Simplex::solve_with_base(ColumnSet const& initial_base) {
        ...
        // Copy costs
        costs = objective_function.getCosts();
    ...
    void Simplex::print_solution() const {
        ...
        for (int i = 0; i < solution_dimension; ++i)
            std::cout << variables.at(i)->getName() << ":\t\t\t" << solution(i) << std::endl;
        ...
        // Generate and solve artificial problem
        {
            ...
            else*/
            {
                ...
                for (int i = 0; i < artificial_problem.solution_dimension; ++i)
                    if (artificial_problem.objective_function.getCosts()(i) == 1 && artificial_problem.current_base.contains(i))
