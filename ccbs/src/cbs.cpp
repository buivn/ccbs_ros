...
Solution CBS::find_solution(const Map &map, const Task &task, const Config &cfg)
{
    ...
    // h_values's length = number of nodes, each sub-vector has agent's number elements.
    // task.get_agents_size() - the number of agents
    for(int i = 0; i < int(task.get_agents_size()); i++) // build the heuristic matrix
    {
        Agent agent = task.get_agent(i);
