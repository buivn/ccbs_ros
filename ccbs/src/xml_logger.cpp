...
void XML_logger::write_to_log_path(const Solution &solution, const Map &map)
{
    ...
    for(int i = 0; i < int(solution.paths.size()); i++)
    {
        ...
        if (i==0) {
            auto it1 = solution.paths[i].nodes.begin();
            std::cout << "The list of pose: " << std::endl;
            while(it1 != std::prev(solution.paths[i].nodes.end()))
            {
                std::cout << CNS_TAG_START_I << ": " <<   map.get_i(it1->id) <<", " <<  CNS_TAG_START_J << ": " << map.get_j(it1->id) << std::endl;
                it1++;
            }
        }
    }
